...
class WalkForwardRequest(BaseModel):
    bars: list[dict[str, float]] = Field(min_length=160, max_length=100000)
    strategy_id: str = "AR-001"
    states: int | str = "auto"
    window: int = Field(default=20, ge=5, le=500)
    seed: int = Field(default=7, ge=0, le=1000000)
    ...
    slippage_bps: float = Field(default=1.0, ge=0, le=1000)
    direction: str = Field(default="long", pattern="^(long|short)$")
    train_pct: float = Field(default=0.60, ge=0.50, lt=0.80)
    test_pct: float = Field(default=0.10, ge=0.05, le=0.25)
    min_state_trades: int = Field(default=5, ge=1, le=100)
...
def walk_forward(request: WalkForwardRequest):
    return run_walk_forward_regime_lab(**request.model_dump())
