...
app = FastAPI(
    ...
    version="0.6.0",
    description=(
        "Trading bot platform for Bitey IA. "
        "Supports demo trading, paper trading, "
        "MT5 demo market data and predefined bot groups. "
...
def validate_config(config: TradingConfig):
    ...
    if config.mode == "live":
        ...
        return {
            ...
            "reason": (
                ...
                "The configuration can be prepared, but no real "
    ...
    if config.execution_provider == "mt5_live":
        ...
        return {
            ...
            "reason": (
                ...
                "Only MT5 demo/read-only market data is currently supported."
            ),
            "config": config.model_dump(),
        }
    ...
    if config.max_position_pct > 0.50:
        ...
        return {
            "valid": False,
            "reason": (
                "max_position_pct cannot exceed 50% of available capital "
                "in the current safety profile."
            ),
            "config": config.model_dump(),
        }
    ...
    if config.currency.upper() not in allowed_currencies:

        return {
            "valid": False,
            "reason": (
                "Unsupported currency. "
                "Supported currencies are USD, BRL and EUR."
            ),
            "config": config.model_dump(),
        }

    # --------------------------------------------------------
    # BOT GROUP VALIDATION
    # --------------------------------------------------------
    ...
    if config.bot_group:
        ...
        if config.bot_group not in valid_groups:
