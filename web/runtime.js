(() => {
  ...
  async function openThesisLab() {
    ...
    const r = await fetch('/thesis-lab.html', { cache:'no-store' }); if (!r.ok) throw new Error('Thesis Lab HTTP '+r.status);
    const wrap = document.createElement('div'); wrap.innerHTML = await r.text(); const section = wrap.querySelector('#thesis-lab-page');
    if (!section) throw new Error('Thesis Lab markup missing'); main.appendChild(section);
    Array.from(wrap.querySelectorAll('script')).forEach(s => { const n=document.createElement('script'); n.textContent=s.textContent; document.body.appendChild(n); }); activateThesisLab();
  }
  function activateThesisLab() {
    closeMobileMenu(); document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));
    const p=document.getElementById('thesis-lab-page'); if(p) p.classList.add('active'); const title=document.getElementById('title'); if(title) title.textContent='Mathematical Thesis Lab';
  ...
  function installThesisNav() {
    ...
    const small=Array.from(nav.querySelectorAll('small')).find(x=>x.textContent.trim().toLowerCase()==='build'); const b=document.createElement('button'); b.dataset.page='thesis-lab'; b.textContent='∑ Mathematical Thesis Lab';
    b.addEventListener('click',()=>openThesisLab().catch(e=>console.error(e))); if(small) small.insertAdjacentElement('afterend',b); else nav.appendChild(b);
  }
  async function openBotLab() {
    if (document.getElementById('bot-lab-page')) return activateBotLab();
    const main = document.querySelector('main.main'); if (!main) return;
    const r = await fetch('/bot-lab.html', { cache:'no-store' }); if (!r.ok) throw new Error('Bot Lab HTTP '+r.status);
    const wrap = document.createElement('div'); wrap.innerHTML = await r.text(); const section = wrap.querySelector('#bot-lab-page');
    if (!section) throw new Error('Bot Lab markup missing'); main.appendChild(section);
    Array.from(wrap.querySelectorAll('script')).forEach(s => { const n=document.createElement('script'); n.textContent=s.textContent; document.body.appendChild(n); }); activateBotLab();
  }
  function closeMobileMenu() {
    const side = document.getElementById('side'); if (!side) return;
    side.classList.remove('open'); side.style.removeProperty('transform'); side.style.removeProperty('pointer-events');
  }
  function activateBotLab() {
    closeMobileMenu(); document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));
    const p=document.getElementById('bot-lab-page'); if(p) p.classList.add('active'); const title=document.getElementById('title'); if(title) title.textContent='Bot Lab';
    document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.page==='bots')); const status=document.getElementById('apiStatus'); if(status) status.textContent='Bot Lab connected · live trading disabled'; loadWebTrader(); loadTradingMode();
  }
  function installBotLabNav() {
    const nav=document.querySelector('.nav'); if(!nav || nav.querySelector('[data-page="bots"]')) return;
    const small=Array.from(nav.querySelectorAll('small')).find(x=>x.textContent.trim().toLowerCase()==='workspace'); const b=document.createElement('button'); b.dataset.page='bots'; b.textContent='◉ Bot Lab';
    b.addEventListener('click',()=>openBotLab().catch(e=>console.error(e))); if(small) small.insertAdjacentElement('afterend',b); else nav.appendChild(b);
  }
  function interceptExistingBotButton() {
    document.querySelectorAll('[data-page="bots"]').forEach(b=>{
      if (b.dataset.botLabWired) return; b.dataset.botLabWired='1';
      b.addEventListener('click',event=>{ event.preventDefault(); event.stopImmediatePropagation(); closeMobileMenu(); openBotLab().catch(e=>console.error(e)); },true);
    });
  }
  function expose() {
