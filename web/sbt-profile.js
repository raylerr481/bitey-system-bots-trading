(() => {
  ...
  const state = {
    ...
    platform: 'mt5',
    execution_mode: 'demo',
    permissions: ['read_market', 'research'],
  ...
  function selectButtons(selector, attribute, value, multi = false) {
    document.querySelectorAll(`${selector} button[${attribute}]`).forEach((button) => {
      if (multi) {
        button.classList.toggle('selected', state.permissions.includes(button.dataset.permission));
      } else {
        button.classList.toggle('selected', button.dataset[attribute.replace('data-', '')] === value);
      }
    });
  }
  ...
  function updateDashboard() {
    ...
    if (user && accessToken()) user.textContent = 'Autenticado';
  }
  ...
  async function loadRegistries() {
    const [ais, platforms] = await Promise.all([
      api('/api/v1/registry/ai/providers'),
      api('/api/v1/registry/platforms')
    ]);
    // Keep the existing visual choices, but make the backend registry authoritative.
    ...
    document.querySelectorAll('#aiChoices button[data-ai]').forEach((b) => {
      const enabled = allowedAI.has(b.dataset.ai);
      b.disabled = !enabled;
    ...
    document.querySelectorAll('#platformChoices button[data-platform]').forEach((b) => {
      const enabled = allowedPlatforms.has(b.dataset.platform);
      b.disabled = !enabled;
      b.style.opacity = enabled ? '' : '.45';
    });
  }
  ...
  async function loadProfile() {
    if (!accessToken()) {
      setStatus('Regístrate para guardar tu entorno');
      updateDashboard();
      return;
    }
    try {
      const profile = await api('/api/v1/user/trading-profile');
      applyProfile(profile);
      setStatus('Entorno cargado');
    } catch (error) {
      setStatus(`Perfil: ${error.message}`);
    }
  }
  ...
  async function saveProfile() {
    ...
    const payload = {
      ...
      ai_connection_mode: state.ai_connection_mode,
      ...
      status: 'configured',
      metadata: { source: 'sbt-web', version: 'registry-v1' }
    };
  ...
  function wireChoices() {
    document.querySelectorAll('#aiChoices button[data-ai]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        document.querySelectorAll('#aiChoices button[data-ai]').forEach(b => b.classList.toggle('selected', b === button));
        updateDashboard();
      });
    });
    document.querySelectorAll('#platformChoices button[data-platform]').forEach((button) => {
      button.addEventListener('click', () => {
        state.platform = button.dataset.platform;
        state.execution_mode = state.platform === 'mt5' ? 'demo' : 'paper';
        document.querySelectorAll('#platformChoices button[data-platform]').forEach(b => b.classList.toggle('selected', b === button));
        updateDashboard();
      });
    });
    document.querySelectorAll('#permissionChoices button[data-permission]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        button.classList.toggle('selected', state.permissions.includes(permission));
      });
    });
  ...
  function wireSave() {
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        button.textContent = 'Validando y guardando…';
        const saved = await saveProfile();
        if (result) {
          result.style.display = 'block';
          result.innerHTML = `<strong>Configuración guardada.</strong><br>IA: ${saved.ai_provider} · Plataforma: ${saved.platform || 'sin seleccionar'} · Modo: ${saved.execution_mode}.<br>Risk Gate: obligatorio · Live trading: BLOQUEADO.`;
        }
        setStatus('Configuración validada');
      } catch (error) {
        if (result) {
          result.style.display = 'block';
          result.style.background = '#2a1414';
          result.style.borderColor = '#673131';
          result.style.color = '#ffb1b1';
          result.textContent = `No se guardó: ${error.message}`;
        }
        setStatus('Validación pendiente');
      } finally {
        button.disabled = false;
        button.textContent = 'Generar plan de implementación';
      }
    });
  }
  ...
  async function init() {
    ...
    try {
      ...
      setStatus(`API no disponible: ${error.message}`);
      wireChoices();
      wireSave();
    }
  }
